...
pub(super) fn do_get_frame(
    ...
    let time = seconds_to_timeline_time(seconds, engine.project().timeline().frame_rate);
    let image = engine
...
fn seconds_to_timeline_time(seconds: f64, rate: Rational) -> RationalTime {
    let ticks = seconds * f64::from(rate.num) / f64::from(rate.den);
    RationalTime::new(ticks.round() as i64, rate)
}
